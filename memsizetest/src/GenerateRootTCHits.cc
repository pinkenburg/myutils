...
GenerateRootTCHits::GenerateRootTCHits(const string &name)
  : SubsysReco(name)
{
  RandomGenerator = gsl_rng_alloc(gsl_rng_mt19937);
}

GenerateRootTCHits::~GenerateRootTCHits()
{
  gsl_rng_free(RandomGenerator);
}

int GenerateRootTCHits::Init(PHCompositeNode *topNode)
{
  return Fun4AllReturnCodes::EVENT_OK;
}

int GenerateRootTCHits::InitRun(PHCompositeNode *topNode)
{
...
int GenerateRootTCHits::process_event(PHCompositeNode *topNode)
{
  if (Verbosity() > 1) cout << "GenerateRootTCHits::process_event -- entered" << endl;
...
int GenerateRootTCHits::End(PHCompositeNode *topNode)
{
  return Fun4AllReturnCodes::EVENT_OK;
}
